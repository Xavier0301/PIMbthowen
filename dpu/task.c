...
int single_discr_kernel() {
    ...
    uint64_t* popcount = (uint64_t*) mem_alloc(ROUND_UP_TO_MULTIPLE_OF_8(sizeof(*popcount)));
        ...
        *popcount = 0;
        ...
        for(unsigned int filter_it = 0; filter_it < model_params.num_filters; ++filter_it) {
            ...
            for(size_t hash_it = 0; hash_it < model_params.filter_hashes; ++hash_it) {
                ...
                uint32_t model_entry_addr = MODEL_ENTRY_ADDR(model_params, mram_base_addr_model, 0, filter_it, hash);
                uint32_t aligned_addr = ROUND_DOWN_TO_MULTIPLE_OF_8(model_entry_addr);
                uint32_t offset = (model_entry_addr - aligned_addr) / sizeof(*filter_buffer);
            ...
            *popcount += (min >= model_params.bleach);
        }

        mram_write(popcount, PREDICTION_ADDR(model_params, mram_base_addr_predictions, sample_it), sizeof(*popcount));
    }
