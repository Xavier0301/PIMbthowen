...
for i in $(seq 0 10); do
    ...
    ./bin/host_code -i 60000 2> /dev/null | grep -a "DPU instructions, "
done
...
# for i in $(seq 0 6); do
#     make clean &> /dev/null
#     PRINT=0 PERF=INSTRUCTIONS NR_TASKLETS=16 NR_DPUS=${DPUS[$i]} make &> /dev/null
#     ./bin/host_code -i $samples_total 2> /dev/null | grep -a "DPU instructions, "
# done
...
#     ./bin/host_code -i $WORK 2> /dev/null | grep -a "DPU instructions, "
# done
